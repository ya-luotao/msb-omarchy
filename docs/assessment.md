...
- **Better fix, upstream**: one line in Hyprland so `m_software` is also true when `GL_RENDERER` contains `llvmpipe` (the GL context exists before `IHyprRenderer` is constructed: `Compositor.cpp:693` vs `:696`), which keeps the four worker threads and only makes the caller wait. Not opened.
