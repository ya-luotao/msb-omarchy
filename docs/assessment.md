...
- **`MSB_GPU=venus`** (`NO_VIRGL | VENUS`, virglrenderer 0.10.4e-krunkit + MoltenVK): initializes without host errors and the guest sees the same device. Whether Venus contexts work was not tested here; it is the subject of M7 in the plan.
...
- **One viewer at a time, properly**: the display server holds a single viewer and `attach()` simply overwrote it, so a second `msb display` (or `bin/display-shot`) left the first window frozen on its last frame, still forwarding input, never exiting. gpu-m3 commit 0f5d0303 keeps a handle on the connection and `shutdown()`s the displaced one; its reader sees EOF and the viewer exits with "sandbox display closed (the sandbox stopped, or another viewer attached)". Verified: viewer 1 exited the moment display-shot connected, viewer 2 started afterwards works.
- **Boot time**: `systemd-analyze` on the 4.0.1 image: kernel 84 ms + userspace 2.83 s, with `ldconfig.service` (348 ms), `systemd-update-done` and friends running on every boot because the image had no `/etc/.updated` stamp (`ConditionNeedsUpdate`). The image now runs `ldconfig`, `systemd-hwdb update`, `journalctl --update-catalog` and touches `/etc/.updated /var/.updated` as its last step: `graphical.target` at 2.30 s. Wall clock from `msb run` returning to the user's `graphical-session.target` active: 5.6 s (polled every 0.5 s). SDDM starts the session 40 ms after logind, so replacing it with agetty autologin would buy nothing measurable.
- **Installing a rebuilt `msb`**: never `cp` over the running binary in place. macOS caches the code signature per vnode; overwriting the same inode leaves every new exec of it killed with SIGKILL (exit 137) until the file is replaced, and the running `msb sandbox`/`msb display` processes can be killed the same way on their next page-in. Copy to `msb.new` and `mv` over the old name instead.
...
## Half-drawn frames (2026-09-03): llvmpipe races the KMS commit

Found by `bin/display-shot`: after a key that only changes a client window (typing into the Omarchy menu, Escape to close it), the guest presents exactly one frame that is stale or partly drawn, then nothing until pointer motion forces another render. `v402-closed.png` differed from the frame before it only in llvmpipe's 64×64 tile row 0, columns 0–319, with a 4-pixel sawtooth boundary inside the tiles: the rasterizer's row-major 4×4-quad shader racing the copy.

- **Mechanism** (Hyprland 0.56.1, mesa 26.2.1): `CHyprGLRenderer::endRender` (`GLRenderer.cpp:111-118`) does `glFinish()` only for NVIDIA or `isSoftware()`, else `glFlush()`, and `Renderer.cpp:2198-2228` commits the atomic state right after. `isSoftware()` (`Renderer.cpp:95-108`) comes from the DRM driver name, which is `virtio_gpu`, never from `GL_RENDERER`, so the llvmpipe guard is dead here. llvmpipe rasterizes asynchronously after `glFlush` (`lp_rast_queue_scene`, `lp_rast.c:1117-1142`: with worker threads the scene is queued and not waited for). Explicit sync would save it (`lp_fence_get_fd` calls `llvmpipe_finish`), but llvmpipe only advertises `EGL_ANDROID_native_fence_sync` when it can open `/dev/udmabuf`, and the guest kernel has no `CONFIG_UDMABUF` (probe in the guest: `native_fence_sync: False`). The kernel's `TRANSFER_TO_HOST_2D` therefore copies a buffer llvmpipe's threads are still writing, the host copies exactly what it is told, and nothing re-flushes when the rasterizer finishes.
- **Not the host**: the copy is byte-exact for the bytes the guest handed over; both slots showed the same partial tiles. Clients are not affected: quickshell/Qt render through wl_shm/drisw, whose swap waits on its fence before the copy (`drisw.c:255-266`); foot uses pixman.
- **First fix (2026-09-03, replaced 2026-09-24, see below)**: `LP_NUM_THREADS=0` for the compositor. With no worker threads `lp_rast_queue_scene` rasterizes inline in the calling thread, so `glFlush()` returns a finished buffer; the color buffer is the mapped dumb buffer itself (`displaytarget_map`), no later copy. `LP_NUM_THREADS=1` is still asynchronous; 0 is the only synchronous mode. Scoped with a drop-in on `wayland-wm@hyprland.desktop.service` (`~/.config/systemd/user/…/llvmpipe.conf`) rather than `environment.d`, because uwsm exports `env.d` into the user manager and would make every Qt client single-threaded too. Cost: rasterization on one vCPU instead of four; see the numbers below.
- **Better fix**: one line in Hyprland so `m_software` is also true when `GL_RENDERER` contains `llvmpipe` (the GL context exists before `IHyprRenderer` is constructed: `Compositor.cpp:693` vs `:696`), which keeps the four worker threads and only makes the caller wait. Hyprland does not accept contributions from unvouched contributors (see "Upstream status" below), so this would have to be carried in the guest image. A variant that needs no compositor rebuild is an `LD_PRELOAD` library for the compositor unit that turns Hyprland's `glFlush()` into `glFinish()` (Hyprland 0.56.1 imports both dynamically from `libGLESv2.so.2`); that is what the image now ships (next section).
- **Numbers**: pointer sweep at 75 moves/s, one frame per move — before the fix 74.8 frames/s (frames that were not finished when copied); after: 14.8 frames/s at 75 moves/s and 15.4 frames/s at 30 moves/s, i.e. one vCPU rasterizes a full 1920×1080 desktop frame in about 65 ms, and every presented frame is complete. `display-shot --key escape` now shows the closed menu, and `--key super+space --key s --key down` shows the query text and the moved highlight, deterministically. Bringing the four threads back needs the Hyprland patch above.

## Half-drawn frames, revisited (2026-09-24): glFinish instead of one thread

`guest/overlay/usr/local/src/msb-glfinish.c` is built into `/usr/local/lib/msb-glfinish.so` and loaded with `LD_PRELOAD` by the compositor unit's drop-in, replacing `LP_NUM_THREADS=0`. Its `glFlush()` resolves to the real `glFinish()` when `/proc/self/comm` is `Hyprland` and to the real `glFlush()` otherwise, and in Hyprland its constructor removes `LD_PRELOAD` from the environment: without that, everything Hyprland starts (quickshell, fcitx5, wayvnc, applications from key bindings) loaded the library too. With it only Hyprland maps it. `glFinish()` waits for llvmpipe's rasterizer threads, so Hyprland's commit sees a finished buffer while llvmpipe keeps one thread per vCPU (four `llvmpipe-N` threads in the compositor, shim mapped, no `LP_NUM_THREADS` in its environment).

Setup: three VMs from the same M4 image (e5423bab), standard profile 1920×1080, 4 vCPUs. `exp-base` is the image as released (`LP_NUM_THREADS=0`); `exp-shim` adds only the shim and the new drop-in; `exp-ctl` is `exp-shim` with the drop-in deleted, i.e. four threads and no fix — the original bug.

- **Correctness.** `bin/frame-check` (new) opens and closes the Omarchy menu through the display protocol and compares the last frame after each close with a closed reference inside the menu's area. `exp-shim`: 30/30 rounds opened the menu, 0 bad frames; an earlier 60-round run of the same procedure: 0 bad frames after closing. With the final library (the `LD_PRELOAD` removal added), 80 further rounds were clean, including 20 right after a restart; one earlier 20-round run flagged 2 frames that were not saved and did not reproduce (4 of its rounds did not open the menu). `exp-ctl`: 28 of 30 rounds flagged; the frames show the menu still on screen after it closed, or bands that end on llvmpipe tile-row boundaries (rows 31, 95, 159, 223, … of the compared area).
- **Speed.** Measured alternately, three rounds each, one workload at a time, while unrelated jobs kept the Mac's load average at 53–76, so absolute numbers are low and noisy; the comparison is what matters.

| Workload | `exp-base` (`LP_NUM_THREADS=0`) | `exp-shim` (4 threads + glFinish) |
| --- | --- | --- |
...
Surveyed 2026-09-24; pull request states and releases rechecked 2026-09-26.

- **microsandbox**: v0.7.3 (2026-09-24) is the latest release; this project's runtime is based on v0.7.2. On `main`, `crates/runtime/lib/vm.rs` moved to `crates/runtime/lib/runner/vm.rs`, `crates/runtime/Cargo.toml` gates `msb_krun` behind a `runner` feature, and `msb_krun` is pinned at `=0.1.39`. This project's draft [#1482](https://github.com/superradcompany/microsandbox/pull/1482) was withdrawn unreviewed on 2026-09-23: at 196 files it was too large to review as one change and had drifted far behind `main`. `--gpu` [#1194](https://github.com/superradcompany/microsandbox/pull/1194) (Venus, by another contributor) is still open. In the GPU issue [#291](https://github.com/superradcompany/microsandbox/issues/291) the maintainers said they have GPU work in progress internally, are considering GPU passthrough, and prefer separate, smaller pull requests. Issue [#1491](https://github.com/superradcompany/microsandbox/issues/1491) asks for clipboard integration, which the fork already has.
- **msb_krun** (superradcompany/libkrun, branch `krun`): 0.1.39 on crates.io. [#116](https://github.com/superradcompany/libkrun/pull/116)–[#119](https://github.com/superradcompany/libkrun/pull/119) were withdrawn with #1482 on 2026-09-23, also unreviewed; none of their changes are in 0.1.39, and no gpu, display or input work landed after 2026-08-28. [#91](https://github.com/superradcompany/libkrun/pull/91) (Venus blob mapping on HVF and KVM, the companion of #1194) is open. The fork therefore carries every change listed in the [findings](findings.md); the standalone ones there are the candidates for small upstream reports.
- **libkrunfw**: no release after v5.2.1; the aarch64 config gained only `CONFIG_VIRTIO_MSB_VMGENID`. Still no `CONFIG_UDMABUF`, so llvmpipe still has no native fences.
- **Hyprland/aquamarine**: aquamarine [#372](https://github.com/hyprwm/aquamarine/pull/372) (cursor-plane hotspot) and Hyprland [#16077](https://github.com/hyprwm/Hyprland/issues/16077) (cursor move schedules a full render) were closed by a bot within seconds: hypr* repositories no longer accept contributions from unvouched contributors, and issues must be discussions. Neither was reviewed. Upstream fixes for the cursor and `glFinish` behaviour described above are therefore not expected; they are this project's to carry or skip. aquamarine 0.15's async commits only help hardware cursors.
- **Arch Linux ARM**: `hyprland 0.56.2-3` depends on and `aquamarine 0.15.1-1` provides `libaquamarine.so=14-64`, so the 2026-09-12 soname mismatch is resolved. mesa is 26.2.3.
- **Omarchy**: the repository moved to `omacom/omarchy` (packages: `omacom/omarchy-pkgs`); 4.0.3 (2026-09-08, security hardening, new default apps) and 4.0.4 (2026-09-15, bespoke kernel) followed 4.0.2. There is still no official aarch64 package repository.

## Runtime on microsandbox v0.7.2: spike (2026-09-24)

Branch `gpu-m4-spike` of the fork (local only, `SPIKE-gpu-m4.md` there) ports `gpu-m3` onto v0.7.2. The vendored patches applied to the 0.1.39 crates with one conflict (`msb_krun_devices/Cargo.toml`); the runtime changes moved into `runner/vm.rs` and `client/ipc.rs`. The Omarchy image boots with display, keyboard, clipboard agent and sound, and stop/start works.

- **Resident pause works with the display devices.** `msb pause`/`msb resume` take about 30 ms each; after 18 s and 90 s pauses the boot ID, the terminal's process and window are unchanged, a key pressed while paused arrives after resume, frames, sound and the clipboard service continue. On macOS the guest's monotonic clock keeps running while paused (0.1.39 stops it only on Windows and Linux/aarch64), so guest timers treat a long pause as elapsed time.
...
- Venus/virgl on Apple Silicon: UTM's `virtio-gpu-gl-pci` freezes (utmapp/UTM#7365); omarchy-arm-utm found GL clients "map but never paint" and shipped ramfb + `LIBGL_ALWAYS_SOFTWARE=1`. The desktop does not depend on acceleration. Venus compute in a microsandbox VM on an M4 Pro has since been shown by libkrun [#91](https://github.com/superradcompany/libkrun/pull/91) (`vulkaninfo`, llama.cpp's Vulkan backend); graphics and display through Venus are untested (plan, M7).
