...
- **Hardware cursor — the device is ready, aquamarine never offers the plane** (2026-08-31): pointer motion alone costs 45.5 full 1920x1080 scanout flushes/s, because the guest draws its pointer into the scanout (the kernel forces full-plane flushes on page flips, `virtgpu_plane.c:91-97`). The device now serves the virtio-gpu cursor queue, and driving the plane directly with `modetest -C` gives **113 cursor images/s and 206 moves/s at 0 frames/s** — the pointer moves for free. Hyprland never gets there:
  - The kernel does expose the plane: `modetest -p` shows plane 33, type 2 (Cursor), format `AR24`, props `HOTSPOT_X`/`HOTSPOT_Y`; the driver caps it at 64x64 `DRM_FORMAT_HOST_ARGB8888`.
  - aquamarine 0.14 enumerates only the primary plane — `drm: Plane 32 has type 1`, and no line for 33 — so it advertises no pointer capability and `/sys/kernel/debug/dri/0/state` keeps `plane[33]` at `crtc=(null) fb=0`.
  - Hyprland therefore never *attempts* a hardware cursor: with `HYPRLAND_TRACE=1`/`AQ_TRACE=1` (2049-line log) there is no "rejected hardware cursors, falling back to sw" trace at all — the fallback is decided before that point.
  - Neither knob helps, both verified in effect at compositor start: `cursor:use_cpu_buffer = 1` (which is the right setting once the plane *is* offered — aquamarine's GBM allocator has no path on a 2D-only virtio-gpu, so the cursor swapchain needs the DRM-dumb allocator) and `cursor:no_hardware_cursors = 0`. Removing `AQ_NO_MODIFIERS` changes nothing either.
  - `WLR_NO_HARDWARE_CURSORS` is a wlroots variable; nothing in Hyprland or aquamarine reads it. Removing it is a no-op and it is gone.
  Both cursor options are tri-states defaulting to `2` (auto), and under Hyprland 0.56's Lua config `hyprctl keyword` is gone ("keyword can't work with non-legacy parsers. Use eval."): it is `hyprctl eval "hl.config({cursor={use_cpu_buffer=1}})"`.
- **Root cause, and a patch that fixes the enumeration** (2026-08-31): virtio_gpu sets `DRIVER_CURSOR_HOTSPOT`, and the kernel hides the cursor plane from any atomic client that has not set `DRM_CLIENT_CAP_CURSOR_PLANE_HOTSPOT` — `drm_mode_getplane_res()` skips it outright, because a client positioning the plane by its top-left corner would misplace the guest pointer. aquamarine 0.14 sets only `UNIVERSAL_PLANES` and `ATOMIC`. `guest/pkgbuilds/aquamarine/aquamarine-cursor-plane-hotspot.patch` sets the cap and programs `HOTSPOT_X`/`HOTSPOT_Y` (the property table already had them; nothing wrote them). Built and installed in a test VM, it works: aquamarine logs `drm: Plane 33 has type 2`, DRM state shows `plane[33]` bound to `crtc-0` with an `AR24` fb, and the device receives `MOVE_CURSOR` at the pointer rate carrying Hyprland's real 64x64 cursor and its `(3,1)` hotspot. `cursor:use_cpu_buffer` was left at its default and was **not** needed — the GBM allocator path works here.
- **But it does not yet pay off, so it is not wired into the image** (2026-08-31): with the patched aquamarine the scanout is still flushed once per pointer move — measured on one boot, same sweep: software cursor **45.5 frames/s**, hardware cursor **73.6 frames/s** against 74 pointer moves/s, i.e. 1:1 with the input rate, plus 73.6 `MOVE_CURSOR`/s. Idle is 0 frames/s either way, so this is caused by pointer motion and not by other damage. The hardware cursor is therefore currently a regression: it adds the cursor traffic without removing the flushes. Where the per-move `RESOURCE_FLUSH` comes from (Hyprland scheduling a frame per cursor move, or the kernel's atomic path pulling the primary plane into a cursor-only commit) is the open question. Until that is answered the patch ships as an artifact only — the image build does not apply it.
- **Cursor pixels lose their alpha** (2026-08-31, open): Hyprland's cursor arrives as a recognisable 64x64 arrow (43 distinct colours) but every pixel reads opaque, so its transparent surround becomes opaque black. The plane's fb is `AR24`, so the alpha is there in the guest; either the device reports the cursor resource in an `X` format (and the runtime's `to_rgba` then forces alpha to 255) or the readback drops it. Must be fixed before a hardware cursor is usable — it would draw a black box around the pointer.
- **Guest session environment**: `systemctl restart sddm` does *not* re-read `/etc/environment.d` — `systemd --user` keeps the old environment and hands it to the new compositor. Use `loginctl terminate-user omarchy` between stopping and starting sddm.
