...
- **microsandbox**: v0.7.2 (2026-09-17); `main` is 229 commits past the `gpu-m0` base. `crates/runtime/lib/vm.rs` moved to `crates/runtime/lib/runner/vm.rs`, `crates/runtime/Cargo.toml` now gates `msb_krun` behind a `runner` feature, and `msb_krun` is pinned at `=0.1.39`. Draft [#1482](https://github.com/superradcompany/microsandbox/pull/1482) has no review and conflicts with `main`; `--gpu` [#1194](https://github.com/superradcompany/microsandbox/pull/1194) has not landed either. Issue [#1491](https://github.com/superradcompany/microsandbox/issues/1491) asks for clipboard integration, which `gpu-m3` already has.
- **msb_krun** (superradcompany/libkrun, branch `krun`): 0.1.39 on crates.io. [#116](https://github.com/superradcompany/libkrun/pull/116)–[#119](https://github.com/superradcompany/libkrun/pull/119) are open with no review; none of their changes are in 0.1.39, and no gpu, display or input work landed after 2026-08-28.
